...
async function unzipBinary(zipPath, destinationDir) {
    const expectedName = path.join(destinationDir, 'humane');
    ...
    throw new Error(`Expecting humane or humane.exe unzipped into ${destinationDir}, didn't find one.`);
}
...
module.exports = async opts => {
    ...
    const assetName = ['humane', opts.version, opts.target].join('-') + '.tar.gz';
    ...
    try {
        ...
        try {
            ...
            const expectedName = path.join(opts.destDir, 'humane');
